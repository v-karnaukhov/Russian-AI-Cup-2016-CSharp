...
namespace Com.CodeGame.CodeWizards2016.DevKit.CSharpCgdk
{
    public sealed class RemoteProcessClient
    {
        ...
        private TE?[] ReadNullableEnums<TE>() where TE : struct, IComparable, IFormattable, IConvertible
        {
            int count = ReadInt();
            if (count < 0)
            {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                enums[i] = value.HasValue ? (TE)Enum.ToObject(typeof(TE), value) : (TE?)null;
            }

            return enums;
        }

        private TE?[][] ReadNullableEnums2D<TE>() where TE : struct, IComparable, IFormattable, IConvertible
        {
            int count = ReadInt();
            if (count < 0)
            {
        ...
        {
            _writer.Write(value ?? -1);
        }

        private void WriteEnums<TE>(TE[] enums) where TE : struct, IComparable, IFormattable, IConvertible
        {
            if (enums == null)
            {
                WriteInt(-1);
        ...
        private void WriteEnums2D<TE>(TE[][] enums) where TE : struct, IComparable, IFormattable, IConvertible
        {
            if (enums == null)
            {
                WriteInt(-1);
        ...
        {
            return _reader.ReadSByte() != 0;
        }

        private void WriteBoolean(bool value)
        {
            _writer.Write((sbyte)(value ? 1 : 0));
        }
